...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ message: 'Success! Email sent.', success: true }), {
      status: 200,
    });
  } catch (error) {
    console.error('Email send error:', error);
    return new Response(JSON.stringify({ message: 'Failed to send email.', success: false }), {
      status: 500,
    });
  }
};
